...
function DigitalFire() {
  ...
  return (
    <div className="relative mt-2 flex flex-col items-center gap-3 overflow-hidden border-t border-amber-500/30 py-10 pl-10">
      ...
      <p className="relative z-10 text-xs tracking-[0.3em] text-neutral-400">{today}</p>
          ...
          return (
            <li key={entry.id} className="group relative py-6 pl-10">
              ...
              <Link href={`/wiki/${entry.slug ?? entry.id}`} className="block">
                ...
                <span className="mt-1 block text-base text-neutral-200 transition-colors group-hover:text-amber-400">
                  {entry.title}
                </span>
              ...
              <div className="pointer-events-none absolute left-full top-4 z-10 ml-6 hidden w-64 border border-neutral-700 bg-neutral-900/95 p-3 shadow-[0_0_30px_-10px_rgba(245,158,11,0.3)] group-hover:block">
                {image ? (
                ...
                <p className="text-xs text-neutral-500">{entry.timeline_date}</p>
              </div>
            </li>
          )
        })}
